...
var config = {};
config.baseURL= serverConfig.sumsubBaseUrl;
...
function createSignature(config) {
  ...
  const signature = crypto.createHmac('sha256',  serverConfig.sumsubSecretKey);
...
function createApplicant(externalUserId, levelName) {
  ...
  var headers = {
      ...
      'X-App-Token': serverConfig.sumsubAppToken
  };
...
function addDocument(applicantId) {
  ...
  var filePath = 'server/resources/sumsub-logo.png';
  ...
  var content = fs.readFileSync(path.join(process.cwd(), filePath)); 
...
export const mainOutdated = async () => {
...
export const main = async ({ externalUserId }) => {
  const levelName = 'basic-kyc-level';
